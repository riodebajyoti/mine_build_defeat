...
export function showTVApps(tv, videoId = '') {
    ...
    const title = document.createElement('h2'); title.textContent = 'YouTube • Guest TV'; title.style.cssText = 'margin:0 0 16px;color:#ff9900;';
    const form = document.createElement('form'); form.style.cssText = 'display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;';
    const input = document.createElement('input'); input.type = 'text'; input.placeholder = 'Paste a YouTube video URL or ID'; input.setAttribute('aria-label', 'YouTube video URL or ID');
    ...
    const message = document.createElement('p'); message.setAttribute('role', 'status'); message.textContent = 'Guest mode: paste a YouTube video link. No API key or game sign-in needed.';
