...
// --- WEATHER SYSTEM ---
let weatherState = 'clear'; // 'clear' | 'rain' | 'blizzard'
let weatherTimer = 90 + Math.random() * 90; // seconds until next change
let weatherTransitionAlpha = 0;
let weatherParticles = null;
let weatherParticleVelocities = [];
let weatherPlayerSpeedMult = 1.0;
let weatherMonsterSpeedMult = 1.0;
...
const WEATHER_CONFIG = {
    clear:    { sky: 0x87CEEB, fog: 0x87CEEB, fogDensity: 0.05, ambient: 1.5, sun: 2.0, hudIcon: '☀️', hudLabel: 'CLEAR',    playerMult: 1.0, monsterMult: 1.0 },
    rain:     { sky: 0x3a4a6b, fog: 0x3a4a6b, fogDensity: 0.07, ambient: 0.7, sun: 0.6, hudIcon: '🌧️', hudLabel: 'RAIN',     playerMult: 1.0, monsterMult: 1.2 },
    blizzard: { sky: 0xc8ddf0, fog: 0xd0e8ff, fogDensity: 0.12, ambient: 0.9, sun: 0.5, hudIcon: '❄️', hudLabel: 'BLIZZARD', playerMult: 0.7, monsterMult: 0.9 },
};

const WEATHER_MESSAGES = {
    clear:    ['☀️ Skies are clearing up.', '☀️ The sun returns.', '☀️ Clear weather ahead.'],
    rain:     ['🌧️ A storm is rolling in...', '🌧️ Rain begins to fall.', '⛈️ Thunder rumbles in the distance.'],
    blizzard: ['❄️ A blizzard sweeps the land.', '❄️ Snow blinds the horizon.', '🌨️ Freezing winds close in...'],
};

function showWeatherBanner(state) {
    const msgs = WEATHER_MESSAGES[state];
    const msg = msgs[Math.floor(Math.random() * msgs.length)];
    weatherBanner.textContent = msg;
    weatherBanner.className = `weather-${state} show`;
    if (bannerTimeout) clearTimeout(bannerTimeout);
    bannerTimeout = setTimeout(() => weatherBanner.classList.remove('show'), 4000);
}

function buildWeatherParticles(state) {
    ...
    weatherParticleVelocities = [];

    if (state === 'clear') return;

    const count = state === 'rain' ? 1200 : 900;
    const spread = 40;
    ...
    for (let i = 0; i < count; i++) {
        ...
        weatherParticleVelocities.push({
            x: state === 'blizzard' ? (Math.random() - 0.5) * 1.5 : 0,
            y: state === 'rain' ? -(8 + Math.random() * 4) : -(1 + Math.random() * 1.5),
            z: state === 'blizzard' ? (Math.random() - 0.5) * 1.5 : 0,
        });
    }
    ...
    const mat = new THREE.PointsMaterial({
        color: state === 'rain' ? 0x7ab8ff : 0xdff0ff,
        size: state === 'rain' ? 0.06 : 0.18,
        transparent: true,
        opacity: state === 'rain' ? 0.7 : 0.85,
        depthWrite: false,
    });
...
function updateWeather(newState, silent = false) {
    const cfg = WEATHER_CONFIG[newState];
    weatherState = newState;
    weatherPlayerSpeedMult = cfg.playerMult;
    weatherMonsterSpeedMult = cfg.monsterMult;

    // Update sky/fog (only apply if it's daytime — don't override night)
    if (worldTime === 'MORNING') {
        scene.background.setHex(cfg.sky);
    ...
    if (weatherHud) weatherHud.textContent = `${cfg.hudIcon} ${cfg.hudLabel}`;

    // Rebuild particles
    buildWeatherParticles(newState);

    if (!silent) showWeatherBanner(newState);
}
...
function animate() {
    ...
    if (state.isPointerLocked) {
        ...
        if (flyMode) {
            ...
            // Normal walking mode
            const effectiveMoveSpeed = MOVE_SPEED * weatherPlayerSpeedMult;
            ...
            if (moveForward || moveBackward) velocity.z -= direction.z * effectiveMoveSpeed * 10.0 * delta;
            if (moveLeft || moveRight) velocity.x -= direction.x * effectiveMoveSpeed * 10.0 * delta;
        ...
        weatherTimer -= delta;
        if (weatherTimer <= 0) {
            const options = ['clear', 'rain', 'blizzard'].filter(w => w !== weatherState);
            const next = options[Math.floor(Math.random() * options.length)];
            updateWeather(next);
            weatherTimer = 90 + Math.random() * 90;
        }

        // Animate weather particles (follow camera so they always surround player)
        if (weatherParticles) {
            weatherParticles.position.x = camera.position.x;
            weatherParticles.position.z = camera.position.z;
            weatherParticles.position.y = camera.position.y + 8;
            ...
            for (let i = 0; i < count; i++) {
                const v = weatherParticleVelocities[i];
                pos.array[i * 3]     += v.x * delta;
                pos.array[i * 3 + 1] += v.y * delta;
                pos.array[i * 3 + 2] += v.z * delta;

                // Wrap particles that fall below origin back to top
            ...
            // Slowly rotate blizzard cloud
            if (weatherState === 'blizzard') {
                weatherParticles.rotation.y += delta * 0.05;
            }
        }
        ...
        monsters.forEach(m => m.update(delta, camera.position, world, weatherMonsterSpeedMult));
